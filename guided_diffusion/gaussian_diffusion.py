...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    #"""
                    if img.shape == (1, 2, 128, 128) or img.shape == (1, 2, 64, 64):
                        pass
                    else:
    ...
    def training_losses_segmentation(self, model, classifier, prior, posterior, x_start , t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            kl_div1= kl.kl_divergence(posterior_latent_space[0], prior_latent_space[0])*0.01
            kl_div2= kl.kl_divergence(posterior_latent_space[1], prior_latent_space[1])*0.1
            kl_div3= kl.kl_divergence(posterior_latent_space[2], prior_latent_space[2])
            
            terms["mse"] = mean_flat((target - model_output) ** 2) #[10,1,128,128]
            terms["kl"] = (kl_div1+kl_div2+kl_div3) / 1000

            
            lamda = 1
            num_consecutive_steps = 2

            if lamda != 0:
                terms['sa']=terms['mse']
                t_k = t
                for k in range(1, num_consecutive_steps):
                    noise_k = th.randn_like(x_start[:, -1:, ...])
                    t_k = torch.clamp(t - k, min=0)
                    mask_k = (t >= k).float()

                    res_t_k = self.q_sample(res, t_k, noise=noise_k)     #add noise to the segmentation channel
                    x_t_k=x_start.float()
                    x_t_k[:, -1:, ...]=res_t_k.float()

                    model_output_k = model(x_t_k, self._scale_timesteps(t_k), **model_kwargs)
                    B, C = x_t_k.shape[:2] #[B=10 batch, C channel ==1 ]
                    C=1
                    assert model_output_k.shape == (B, C * 2, *x_t_k.shape[2:])
                    model_output_k, model_var_values = th.split(model_output_k, C, dim=1)

                    target_k = {
                        ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                            x_start=res, x_t=res_t_k, t=t_k
                        )[0],
                        ModelMeanType.START_X: res,
                        ModelMeanType.EPSILON: noise_k,
                    }[self.model_mean_type]

                    terms["sa"] += mean_flat((target_k - model_output_k) ** 2)*mask_k
                terms['sa']*=1/(num_consecutive_steps**2)

            weight = 0.01
            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"] + weight * terms["kl"]+ (1-weight) * lamda*terms['sa']
            else:
                terms["loss"] = terms["mse"] + weight * terms["kl"] + (1-weight)*lamda*terms['sa']
