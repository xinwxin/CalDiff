...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        if self.resume_step:
            self.prior.load_state_dict(th.load(self.resume_checkpoint.split('.pt',0)+'prior.pt'))
            self.posterior.load_state_dict(th.load(self.resume_checkpoint.split('.pt',0)+'posterior.pt'))
    ...
    def run_loop(self):
        ...
        while self.step <= self.train_stop:
    ...
    def save(self):
        ...
        f1 = f"savedmodel{(self.step+self.resume_step):06d}prior.pt"
        with bf.BlobFile(bf.join(get_blob_logdir(), f1), "wb") as f1:
            th.save(self.prior.state_dict(), f1)
        f2 = f"savedmodel{(self.step+self.resume_step):06d}posterior.pt"
        with bf.BlobFile(bf.join(get_blob_logdir(), f2), "wb") as f2:
            th.save(self.posterior.state_dict(), f2)
